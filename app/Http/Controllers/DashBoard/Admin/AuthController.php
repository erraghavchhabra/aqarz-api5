...
class AuthController extends Controller
{
    ...
    public function update_password(Request $request)
    {
        ...
        $admin = auth()->guard('Admin')->user();
        if (!\Hash::check($request->get('old_password'), $admin->password)) {
            return response()->error(__('views.old password incorrect'));
        }

        if ($request->get('new_password') == $request->get('old_password')) {
            return response()->error(__('views.new password must be different from old password'));
        }
        $user = Admin::find($admin->id);
    ...
    public function settings()
    {
        ...
        return response()->success(__('views.Settings'), $settings);
    }


    public function changeDaysBeta(Request $request)
    {
        ...
        $settings->count_beta_days = $request->get('days_number');
        $settings->save();
        return response()->success(__('views.Settings'), $settings);
    }
