...
use App\Http\Resources\v4\RateRequestResource;
 ...
 */
    ...
    public function addEstate(Request $request)
    {
        ...
        if ($request->city_id && !$request->neighborhood_id && !$request->ios_address) {
        ...
        } elseif ($request->neighborhood_id && !$request->ios_address) {
        ...
        if ($request->ios_address) {
            $request->merge([
                'full_address' => $request->ios_address,
            ]);
        }
...
//////
    ...
    */
        ...
        if ($request->neighborhood_name_request) {
            $request->merge([
...
//////
    ...
    public function updateStatistics(Request $request, $id)
    {
        ...
        $rules = Validator::make($request->all(), [
           'type' => 'required|in:show_video_count,message_count,message_whatsapp_count,share_count,screenshot_count,call_count',
        ...
        if ($request->type == 'message_count') {
            $estate->message_count = $estate->message_count + 1;
        }

        if ($request->type == 'message_whatsapp_count') {
            $estate->message_whatsapp_count = $estate->message_whatsapp_count + 1;
        }

        if ($request->type == 'share_count') {
            $estate->share_count = $estate->share_count + 1;
        }

        if ($request->type == 'screenshot_count') {
            $estate->screenshot_count = $estate->screenshot_count + 1;
        }

        if ($request->type == 'show_video_count') {
            $estate->show_video_count = $estate->show_video_count + 1;
        }

        if ($request->type == 'location_count') {
            $estate->location_count = $estate->location_count + 1;
        }

        if ($request->type == 'call_count') {
            $estate->call_count = $estate->call_count + 1;
        }
...
//////
    ...
    public function show_rate_Request()
    {
        $user = auth()->user();
        if ($user == null) {
            return response()->error(__("views.not authorized"));
        }
        $RateRequest = RateRequest::where('user_id', $user->id)->paginate();
        $RateRequest = RateRequestResource::collection($RateRequest)->response()->getData(true);

        return response()->success(__("views.ShowRateRequest"), $RateRequest);
    }

    public function show_rate_Request_details($id)
    {
        $user = auth()->user();
        if ($user == null) {
            return response()->error(__("views.not authorized"));
        }
        $RateRequest = RateRequest::find($id);
        if (!$RateRequest) {
            return response()->error(__("views.not found"));
        }
        $RateRequest = new RateRequestShowResource($RateRequest);

        return response()->success(__("views.ShowRateRequest"), $RateRequest);
    }

    public function offer_rate_Request_change_status(Request $request)
    {
        $user = auth()->user();
        if ($user == null) {
            return response()->error(__("views.not authorized"));
        ...
        $rules = Validator::make($request->all(), [
            'offer_rate_id' => 'required|exists:rate_offer,id',
        ...
        $offer = RateOfferRequest::find($request->offer_rate_id);
        if ($offer->status != 'pending') {
            return response()->error(__("views.offer not pending"), []);
        }
        ...
        if ($request->status == 'accepted') {
            $RateRequest = RateRequest::find($offer->request_rate_id);
            $RateRequest->status = 'completed';
            $RateRequest->save();

            $get_all_offer = RateOfferRequest::where('request_rate_id', $offer->rate_request_id)->where('id', '!=', $offer->id)->get();
            foreach ($get_all_offer as $item) {
    ...
    public function delete_rate_Request($id)
    {
        $user = auth()->user();
        if ($user == null) {
            return response()->error(__("views.not authorized"));
        ...
        if ($RateRequest) {
            $RateRequest->delete();
            return response()->success(__("views.DeleteRateRequest"), []);
        } else {
            return response()->error(__("views.not found"), []);
        }
    }
