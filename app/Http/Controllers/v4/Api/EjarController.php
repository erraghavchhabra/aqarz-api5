...
class EjarController extends Controller
{
    ...
    public function add_properties(Request $request)
    {
        $rules = Validator::make($request->all(), [
            ...
            'latitude' => 'required_if:estate_id,==,null',
            'longitude' => 'required_if:estate_id,==,null',
            'kitchen_number' => 'required_if:estate_id,==,null',
            'elevators_number' => 'required_if:estate_id,==,null',
            'parking_spaces_numbers' => 'required_if:estate_id,!=,null',
...
//        $estate_name = $estate->estate_type_name . ' - ' . $estate->city_name . ' - ' . $estate->neighborhood_name . ' - ' . $estate->street_name . ' - #' . $estate->id;
            ...
            $estate_name = Carbon::now()->timestamp;
        }
        ...
        $data = [
            ...
            'latitude' => $request->estate_id ? $estate->lat : $request->latitude,
            'longitude' => $request->estate_id ? $estate->lan : $request->longitude,
            'contract_type' => 'residential',
            'kitchen_number' => $request->kitchen_number ?? 0,
            ...
            'ejar_parking_spaces' => $request->estate_id ? $estate->parking_spaces_numbers ?? 0 : $request->parking_spaces_numbers,
            'ejar_security_entries' => $request->get('ejar_security_entries') ?? 0,
            'ejar_banquet_hall' => $request->get('ejar_banquet_hall') ?? 0,
            'ejar_elevators' => $request->estate_id ? $estate->elevators_number ?? 0 : $request->elevators_number,
            ...
            'ejar_cafeteria' => $request->estate_id ? $estate->kitchen_number ?? 0 : $request->kitchen_number,
            ...
            'ejar_compound_name' => $request->estate_id ? $estate->compound_name ?? '' : $request->ejar_compound_name ?? '',
        ];
        ...
        if ($id) {
            $ejar_info = EjarEstateData::create([
                ...
                'kitchen_number' => $request->kitchen_number,
...
//            ]);
    ...
    public function add_unit(Request $request)
    {
        ...
        $rules = Validator::make($request->all(), [
            'estate_id' => 'required_if:properties_id,==,null|exists:estates,id',
            'ejar_storeroom' => '',
            'ejar_central_ac' => '',
            'ejar_desert_cooler' => '',
            'ejar_split_unit' => '',
            'ejar_backyard' => '',
            'ejar_maid_room' => '',
            'ejar_is_kitchen_sink_installed' => 'required',
            'ejar_is_cabinet_installed' => 'required',
            ...
            'properties_id' => 'required_if:estate_id,==,null',
            'ejar_direction' => 'required_if:estate_id,==,null|in:north,south,east,west',
            'ejar_bedrooms' => '',
            'ejar_bathrooms_number' => '',
            'ejar_halls_number' => '',
            'ejar_majles_number' => '',
        ]);
        ...
        if ($request->estate_id) {
            ...
            $check_exist_ejar = EjarEstateData::where('properties_id', $request->get('properties_id'))->first();
            if (!$check_exist_ejar) {
                return JsonResponse::fail(['error' => 'properties not exist'], 400);
            }
            $direction = $request->ejar_direction;
        }


        $bedrooms = $request->estate_id ? $estate->bedroom_number ?? 0 : $request->ejar_bedrooms_number ?? 0;
        $bathrooms_full = $request->estate_id ? $estate->bathroom_number ?? 0 : $request->ejar_bathrooms_number ?? 0;
        $halls = $request->estate_id ? $estate->lounges_number ?? 0 : $request->ejar_halls_number ?? 0;
        $storeroom = $request->ejar_storeroom ?? 0;
        $central_ac = $request->ejar_central_ac ?? 0;
        $kitchen = $check_exist_ejar->kitchen_number ?? 0;
        $majles = $request->estate_id ? $estate->boards_number ?? 0 : $request->ejar_majles_number ?? 0;
        ...
        $floor_number = $request->estate_id ? $estate->floor_number ?? 0 : $request->ejar_floor_number ?? 0;
        ...
        $area = $request->estate_id ? $estate->total_area ?? 0 : $request->ejar_total_area ?? 0;
        $established_date = $request->ejar_established_date;
...
//        }
    ...
    public function contract_unit_services_contract(Request $request, $contract_id)
    {
        ...
        return response()->success(__("views.success"), []);
    ...
    public function rental_fee_contract(Request $request, $contract_id)
    {
        ...
        $rules = Validator::make($request->all(), [
            ...
            'payment_type' => 'required|in:annually,monthly,with_payments,one_time',
        ]);
