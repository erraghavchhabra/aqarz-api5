...
use App\Models\v2\OprationType;
use App\Models\v2\RateRequestType;
use App\Models\v2\Region;
use App\Models\v2\Report;
use App\Models\v2\RequestFund;
use App\Models\v2\RequestOffer;
use App\Models\v2\ServiceType;
use App\Models\v2\StreetView;
...
//return [json_decode($body)];
    ...
    public function settings()
    {
        ...
        /* $users=User::query()->get();
         ...
         {
             $offer = Estate::where('user_id', $uItem->id)->count();




             $checkOffer = RequestFund::whereHas('offers', function ($query) use ($uItem) {
                 $query->where('provider_id',$uItem->id);
             })->count();



             $uItem->count_offer = $offer;
             $uItem->count_request = $checkOffer;
             $uItem->save();
         }


        /* $request_offer = RequestFund::where('id', '>', 0)->get();

         foreach ($request_offer as $rItem) {
             $offer = FundRequestOffer::where('uuid', $rItem->uuid)->count();
             $rItem->count_offers = $offer;
             $rItem->save();
         }*/

        /*  $request = RequestFund::where('id', '>', 0)->pluck('uuid');
          dd($request->toArray());
          $offer = FundRequestHasOffer::whereNotIN('uuid', $request->toArray())->delete();
  */
...
*/
    ...
    public function GolobalCites(Request $request)
    {
        ...
        //     $users = \App\Models\v2\District::where('city_id',3)->get()->toJson();
        $cities = \App\Models\v2\City3::query();
    ...
    public function GolobalNeb(Request $request, $id)
    {
        ...
        $neb = \App\Models\v2\District::where('city_id', $id);
        //  $cities = \App\Models\v2\City3::query();
...
//dd($request->all());
        ...
        if ($request->get('is_all') != '1' && $request->get('is_all') != '2' && $request->get('is_all') != '3' && $request->get('is_all') == '0') {
            ...
            $Neighborhood = $Neighborhood->whereHas('requestFund');
        }
