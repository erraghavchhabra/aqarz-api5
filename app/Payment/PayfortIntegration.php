...
class PayfortIntegration
{
     ...
     */
    public $merchantIdentifier = 'c4e02858';
    public $uuid = '';

    /**
     * @var string your access code
     */
    public $accessCode = 'pkLUheSqvbMvxFKmwyF2';

    /**
     * @var string SHA Request passphrase
     */
    public $SHARequestPhrase = '$2y$10$PbBSlejqG';

    /**
     * @var string SHA Response passphrase
     */
    public $SHAResponsePhrase = '$2y$10$qLf/VJTpP';
    ...
    public function getRedirectionData($paymentMethod)
    {
        ...
        $postData = [
            ...
            'return_url'          => $this->getUrl('get/payment/page?r=processResponse'),
        ];
    ...
    public function getMerchantPageData($paymentMethod, $uuid)
    {
        ...
        $returnUrl = $this->getUrl('get/payment/page?r=merchantPageReturn&3ds=' . $uuid);
        if (isset($_GET['3ds']) && $_GET['3ds'] == 'no') {
            $returnUrl = $this->getUrl('get/payment/page?r=merchantPageReturn&3ds=no');
        }
        $iframeParams = [
            'merchant_identifier' => $this->merchantIdentifier,
            'access_code'         => $this->accessCode,
    ...
    public function processResponse()
    {
        ...
        if (empty($fortParams)) {
            ...
            if ($responseSignature != $calculatedSignature) {
                ...
                $debugMsg = sprintf('Invalid Signature. Calculated Signature: %1s, Response Signature: %2s',
                    $responseSignature, $calculatedSignature);
                $this->log($debugMsg);
            }
            ...
            if ($responseSignature != $calculatedSignature) {
                ...
                if (substr($response_code, 2) != '000') {
                    ...
                    if (!$host2HostParams) {
                        ...
                        $user_plan = UserPlan::find($this->user_plan_id);


                        if ($user_plan) {
                            $user_plan->fort_id = $host2HostParams['fort_id'];
                            $user_plan->save();
                        }
