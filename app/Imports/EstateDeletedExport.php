...
class EstateDeletedExport implements FromView, WithColumnWidths
{
    ...
    public function view(): View
    {
        ...
        $create_report = \App\Models\v3\Estate::withTrashed()
            ->where('deleted_at', '!=', null)
            ->where('user_id', '!=', 142)
            ->where('id', '!=', 2243)
            ->get();

        foreach ($create_report as $create_reportItem) {
            $checkReprot = Report::where('estate_id', $create_reportItem->id)
                ->first();
            if (!$checkReprot) {
                $reprot = Report::create([
                    'body' => 'العقار غير موافق لشروط',
                    'user_id' => 3417,
                    'estate_id' => $create_reportItem->id,
                    'report_type' => 'error_data',
                ]);
            }

            if (!$create_reportItem->reason) {
                $create_reportItem->reason = 'مشكلة بالتواصل';
                $create_reportItem->save();
            }
        }
        return view('EstateDeletedExport', [
            'estate' => \App\Models\v3\Estate::withTrashed()
                ->where('deleted_at', '!=', null)
                ->where('user_id', '!=', 142)
                ->where('id', '!=', 2243)
                ->get()
    ...
    public function columnWidths(): array
    {
        return [
            ...
            'AY' => 100,//Advertiser's email
            'AZ' => 100,//Advertiser's email
            'BA' => 100,//Advertiser's email
            'BB' => 300,//Advertiser's email
