...
class EstateDaliyExport implements FromView, WithColumnWidths
{
   ...
   //     $date= Carbon::parse(date('Y-m-d'));
    ...
    /*    return view('EstateDaliyExport', [
            'estate' => \App\Models\v3\Estate::where('is_complete','1')
        ...
        return view('EstateDaliyExport', [
            ...
            'estate' => \App\Models\v3\Estate::where('is_complete','1')->where('status' , 'completed')
                //->whereNotIn('id',$ids->toArray())
               // ->where('id','>',0)
//                ->whereDate('created_at', '<=', $date)
